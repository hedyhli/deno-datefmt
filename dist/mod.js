...
const getWithUTC = (date, fn, utc) => {
    // deno-lint-ignore no-explicit-any
    return date['get' + (utc ? 'UTC' : '') + fn]();
};
const pad0 = (d) => (d.length === 1 ? '0' : '') + d;
const datefmt = (date, fmt = '[2006]-[01]-[02]', utc = true, delim = '[]') => {
    if (delim.length > 2 || delim.length == 0) {
        throw new Error("'delim' argument must either have one or two characters.");
    }
    let s = "";
    const openD = delim[0], closeD = delim.length == 1 ? openD : delim[1];
    if (fmt == openD + closeD) {
        return openD + closeD;
    }
...
exports.default = exports.datefmt;
